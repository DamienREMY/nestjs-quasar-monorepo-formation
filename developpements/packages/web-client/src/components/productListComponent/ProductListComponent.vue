...
<q-form @submit="onSubmit('Recherche finalisée')" @reset="onReset()" class="filter-form">
    <q-input class="filter-input"
      ...
      :rules="[
        (val) =>
        (val && val.length > 0) || 'Veuillez entrer un libellé valide !']"

    />

    <q-btn class="filter-button"
      color="primary"
      icon="fa fa-search"
      label="Rechercher"
      type="submit"
      @click="queryProductfromLibelle(libelle)"
    />
