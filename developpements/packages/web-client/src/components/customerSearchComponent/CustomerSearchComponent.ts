...
export default defineComponent({
  ...
  setup() {
    ...
    const searchAllResponse = ref<IPaginatedListDto<CustomerSearchResultDto>>({
      list: [],
      page: 1,
    });
    ...
    function doCheckForSearch(searchAllParams: ISearchDto<SearchCustomerDto>) {
      if (
        !!searchAllParams.criterias?.chronoClient ||
        !!searchAllParams.criterias?.nom ||
        !!searchAllParams.criterias?.prenom ||
        !!searchAllParams.criterias?.codePostal ||
        !!searchAllParams.criterias?.ville ||
        !!searchAllParams.criterias?.dateDerniereCommandeTo ||
        !!searchAllParams.criterias?.dateDerniereCommandeFrom
      ) {
        return true;
      }
      return false;
    ...
    async function _doSearchAll(
      searchAllParams: ISearchDto<SearchCustomerDto>,
    ): Promise<WorkDone<boolean>> {
      loading.value = true;
      const wd = await customersApiService.getCustomerListByCriterias(
        searchAllParams,
      );
      if (wd.isOk && !!wd.data) {
    ...
    function filterFichPart(val: string, update: any) {
      ...
      update(() => {
        ...
        fichPartOptions.value = allFichierPartenaires.filter(
